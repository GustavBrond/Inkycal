...
class DashedImageDraw(ImageDraw.ImageDraw):
    ...
    def dashed_line(self, xy, dash=(2,2), fill=None, width=0, invert = False):
        #xy – Sequence of 2-tuples like [(x, y), (x, y), ...]
...
def draw_border(image: Image, xy: Tuple[int, int], size: Tuple[int, int], radius: int = 5, thickness: int = 1,
                shrinkage: Tuple[int, int] = (0.1, 0.1), dashed=False, dash = (10,10), invert = False) -> None:
    ...
    if (dashed):
        ...
        draw.dashed_line((p1, p2), fill=colour, width=thickness, dash=dash, invert = invert)
        draw.dashed_line((p3, p4), fill=colour, width=thickness, dash=dash, invert = invert)
        draw.dashed_line((p5, p6), fill=colour, width=thickness, dash=dash, invert = invert)
        draw.dashed_line((p7, p8), fill=colour, width=thickness, dash=dash, invert = invert)
